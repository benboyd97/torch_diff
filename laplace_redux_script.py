...
mnist_train_loader, mnist_test_loader = get_mnist_loaders(batch_size=128,test_batch_size=10000)
...
hess_type = ['lowrank']
samp_type = ['linear','samp']
prior_prec = [1.0,10.0,1e-1,1e-4, 1e-3, 1e-2,100.0]
    ...
    for st in samp_type:
        for subset in ['all']:
            for p in prior_prec:
                ...
                if (st == 'linear' and ht == 'lowrank') or (subset == 'last_layer' and ht == 'lowrank'): 
                    print('skip')
                else:
                    print(ht,st,subset,str(p))
                    ...
                    filename = f'laplace_redux_dicts/redux_{subset}_{ht}_{st}_prior{str(p)}.json'
